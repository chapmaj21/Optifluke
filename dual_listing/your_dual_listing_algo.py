...
while True:
    ...
    print(
        f"Top level prices for {stock_id}: {best_bid_price:.2f} :: {best_ask_price:.2f}"
    )
    ...
    if not trade_would_breach_position_limit(stock_id, volume, side):
        print(
            f"""Inserting {side} for {stock_id}: {volume:.0f} lot(s) at price {price:.2f}."""
        )
        ...
        print(
            f"""Not inserting {volume:.0f} lot {side} for {stock_id} to avoid position-limit breach."""
        )
